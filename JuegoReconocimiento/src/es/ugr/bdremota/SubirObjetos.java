...
class SubirObjetos extends AsyncTask<List<String>, String, String> {
	...
	public SubirObjetos(Context context) {
		...
		url_create_objeto = context.getString(R.string.servidor_remoto)
				+ "create_objeto.php";
		url_update_objeto = context.getString(R.string.servidor_remoto)
				+ "update_objeto.php";

		path_imagen_rem = context.getString(R.string.servidor_remoto)
				+ "images/";
		path_sound_rem = context.getString(R.string.servidor_remoto)
				+ "sounds/";
	...
	protected String doInBackground(List<String>... args) {
		...
		return "Subidos: " + args[0].size() + " nuevos objetos; Actualizados: "
				+ args[1].size() + " objetos.";
	}
	...
	private void InsertarModificar(Objeto obj, boolean insertar) {
		...
		if (!obj.getSonidoDescripcion().equals("")) {
			...
			nombre_fich = nombre_fich.replaceAll("\\s+", "_"); // sustituye
																// espacios por
																// barras_baja
			new SubirFicherosSFTP(context).execute(obj.getSonidoDescripcion(),
					nombre_fich, "sounds");
			params.add(new BasicNameValuePair("sonido_descripcion",
					path_sound_rem + nombre_fich));
		}
		...
		if (!obj.getSonidoAyuda().equals("")) {
			...
			nombre_fich = nombre_fich.replaceAll("\\s+", "_"); // sustituye
																// espacios por
																// barras_baja
			new SubirFicherosSFTP(context).execute(obj.getSonidoAyuda(),
					nombre_fich, "sounds");
			params.add(new BasicNameValuePair("sonido_ayuda", path_sound_rem
					+ nombre_fich));
		}
		...
		if (!obj.getSonidoNombre().equals("")) {
			...
			nombre_fich = nombre_fich.replaceAll("\\s+", "_"); // sustituye
																// espacios por
																// barras_baja
			new SubirFicherosSFTP(context).execute(obj.getSonidoNombre(),
					nombre_fich, "sounds");
			params.add(new BasicNameValuePair("sonido_nombre", path_sound_rem
					+ nombre_fich));
		}
		...
		if (!obj.getPathImagen().equals("")) {
			...
			nombre_fich = nombre_fich.replaceAll("\\s+", "_"); // sustituye
																// espacios por
																// barras_baja
			new SubirFicherosSFTP(context).execute(obj.getPathImagen(),
					nombre_fich, "images");
			// funcion();
			params.add(new BasicNameValuePair("imagen", path_imagen_rem
					+ nombre_fich));
		}
		...
		if (insertar == true)
			json = jParser.makeHttpRequest(url_create_objeto, "POST", params);
		else
			json = jParser.makeHttpRequest(url_update_objeto, "POST", params);
